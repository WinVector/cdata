...
[`cdata`](https://winvector.github.io/cdata/) supplies a few tools for creating example 
...
Notice how the table is specified by column (which is close to how `data.frame`s
...
`cdata` supplies the method [`draw_frame`](https://winvector.github.io/cdata/reference/draw_frame.html) which at first glance appears to be a mere pretty-printer:
...
However, the above code is actually executable `R` code.  If we run it, we re-create
...
`build_frame()` allows for simple substitutions of values.  In contract the method `qchar_frame()`
