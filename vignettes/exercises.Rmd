...
demonstrate data layout transforms using the [`cdata`](https://CRAN.R-project.org/package=cdata) [`R`](https://www.r-project.org) package.  The examples for this note are all demo-examples from [tidyr/demo/](https://github.com/tidyverse/tidyr/tree/master/demo), and are mostly based on questions posted to StackOverflow. They represent a good cross-section of data layout problems, so they are a good set of examples or exercises to work through.
...
Each of these five problems will be solved using the same steps:
  ...
  * Check if either the incoming or outgoing data format is in "row-record" format: is all the data for a single record contained in one row? This determines which specification you will use:
  ...
  * Identify which columns form the record ids, which we call the "record keys."
...
Each record represents a family, and is designated by the record key `famid`. The data starts with each record in a single row (a row record):
...
However, `outgoing_record` is just a data frame, you can create it however you like -- you don't need to use `qchar_frame()`.
...
First, notice that neither the ingoing nor outgoing forms are single-row records; a single record corresponds to a single
...
Since neither the ingoing nor outgoing shapes are row records, we use the general `layout_specification()`.
