...
First define the control table specifying what a single row looks like after transformation.
...
knitr::kable(control_table)
```

And then have `cdata::rowrecs_to_blocks()` perform the transform.  Notice we are specifying that the columns `Part` and `Measure` are the control table row-keys.
...
And, as one comes to expect with coordinatized/fluid-data transforms, the process is easy to reverse.
