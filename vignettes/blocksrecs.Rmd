...
The coordinatized data concept: is the exact implementation is not important.  One can perform a data *change of basis* to get data into the right format (where the physical layout of records is altered to match the desired logical layout of the data).

The idea of data records records, and these records possibly being different than simple rows is a staple of computer science: harking at least back to [record-oriented filesystems](https://en.wikipedia.org/wiki/Record-oriented_filesystem), and perhaps living on in JSON nested record.

The entirety of the `cdata` package is to supply a transforms between what we call "row records" (records that
...
Let's look at this with some specific data.
...
```{r}
iris$row_id <- seq_len(nrow(iris))

iris_aug <- rowrecs_to_blocks(
  iris,
  controlTable,
  columnsToCopy = c("row_id", "Species"))

head(iris_aug, n=6)

iris_back <- blocks_to_rowrecs(
  iris_aug,
  keyColumns = c("row_id", "Species"),
  controlTable
)
...
The `cdata` operators have some interesting lineage
  ...
  * `blocks_to_rowrecs()` is essential an aggregation or relational-projection (actually even a coalesce) over a widened table.  This is why this operation decreases the number of rows.
...
Some additonal tutorials on `cdata` data transforms can are given below:
