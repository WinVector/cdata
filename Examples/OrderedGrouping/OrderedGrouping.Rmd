---
title: "Ordered Grouping Example"
output: github_document
---

## Introduction

I'd like to share an example of data-wrangling/data-reshaping and how to solve it using [`rqdatatable`](https://github.com/WinVector/rqdatatable/)/[`cdata`](https://github.com/WinVector/cdata/).
...
```{r, echo=FALSE}
knitr::kable(d2[d2$ID==1, , drop=FALSE])
```
...
The `cdata` data shaping rule is: draw a picture of any non-trivial (more than one row) data records in their full generality.  In our case the interesting record is the following.
...
The column-names `rank`, `DATE`, and `OP` are all column names of the table we are starting with.  The values `1`, `2`, and `3` are all values we expect to see in the `rank` column of the working data frame.  And the symbols `DATE1`, `DATE2`, `DATE3`, `OP1`, `OP2`, and `OP3` are all stand-in names for values we see in our data.
